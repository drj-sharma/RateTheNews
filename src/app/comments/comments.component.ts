...
export class CommentsComponent implements OnInit {
  ...
  getComments(){
    var parent = this; 
    console.log(this.i)
    this.db.collection("comments",ref => ref.orderBy('time').startAfter(this.tempdate).limit(this.i)).get().toPromise().then( (querySnapshot) => {
      querySnapshot.forEach(function(doc) {
        ...
        parent.comments[parent.comments.length-1].tempreplydate = new Date("July 21, 1993 01:15:00");
  ...
  getreplies(arrayItem){
    var parent = this;
      parent.db.collection("replies", ref => ref.where('commentid', '==' ,arrayItem.commentid).orderBy('time').startAfter(arrayItem.tempreplydate).limit(5)).get().toPromise().then( (querySnapshot) => {
      querySnapshot.forEach(function(doc) {
  ...
  getuserdata(){
    ...
    this.comments.forEach(function (arrayItem) {
      arrayItem['replies'].forEach(element => {
        var docRef = parent.db.collection("users").doc(element.uid);
